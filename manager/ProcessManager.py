...
'''
Process managers for fonero-daemon, fonero-wallet-cli and fonero-wallet-rpc
'''
...
from settings import WALLET_RPC_HOST, WALLET_RPC_PORT, REMOTE_DAEMON_HOST, REMOTE_DAEMON_PORT
...
class WalletRPCManager(ProcessManager):
    ...
    def run(self):
        rpc_ready_strs = ["Binding on 127.0.0.1:%s" % WALLET_RPC_PORT, "Starting wallet rpc server", "Run net_service loop", "Refresh done"]
        err_str = "ERROR"
        ...
        for line in iter(self.proc.stdout.readline, b''):
            ...
            self.last_log_lines.append(line[:240])
