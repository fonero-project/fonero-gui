...
class Hub(QObject):
    ...
    def __init__(self, app):
        ...
        self.app = app
        self.log_file = os.path.join(DATA_DIR, 'logs', "fonero-wallet-rpc.log")
    ...
    def create_new_wallet(self, mnemonic_seed=u'', restore_height=0):
        ...
        try:
            ...
            if has_password:
                ...
                wallet_log_path = os.path.join(wallet_dir_path, 'fonero-wallet-cli.log')
                ...
                if not mnemonic_seed: # i.e. create new wallet
                    ...
                    self.wallet_cli_manager = WalletCliManager(resources_path, wallet_filepath, wallet_log_path, True, restore_height)
                    self.wallet_cli_manager.start()
                    self.app_process_events(1)
    ...
    def send_tx(self, amount, address, payment_id, priority, mixin, tx_desc, save_address, sweep_all):
        ...
        if sweep_all:
            _, _ = self.ui.wallet_rpc_manager.rpc_request.get_balance()
            ret = self.ui.wallet_rpc_manager.rpc_request.transfer_all(address, payment_id, priority, mixin)
        else:  
            ret = self.ui.wallet_rpc_manager.rpc_request.transfer_split(amount, address, payment_id, priority, mixin)            
    ...
    def _show_wallet_info(self):
        ...
        #self.app_process_events()       
        #self.on_wallet_update_info_event.emit(json.dumps(wallet_info))
        #self.app_process_events()
