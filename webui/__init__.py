...
MAX_NEW_SUBADDRESSES = 0
tray_icon_tooltip = "%s v%s.%s.%s" % (APP_NAME, VERSION[0], VERSION[1], VERSION[2])
...
#             + os.path.join(self.app.property("ResPath"), "www", self.html).replace('\\', '/')
        ...
        self.icon = self._getQIcon('fonero_64x64.png')
...
class NewWalletWebUI(BaseWebUI):
    def __init__(self, app, hub, debug):
        window_size = qt_core.QSize(800, 623)
...
class MainWebUI(BaseWebUI):
    def __init__(self, app, hub, debug):
        ...
        if sys.platform == 'darwin':
            tray_icon = 'fonero_16x16_mac.png'
        elif sys.platform == "win32":
            tray_icon = 'fonero_16x16.png'
        else:
            tray_icon = 'fonero_32x32_ubuntu.png'
...
#         self.view.load(qt_core.QUrl(self.url))
    ...
    def run_wallet_rpc(self, wallet_password, log_level=1):
        ...
        while True:
            ...
            daemon_info = self.daemon_rpc_request.get_info()
            if daemon_info['status'] == "OK":
    ...
    def _update_daemon_status(self):
        ...
        if daemon_info['status'] == "OK":
            status = "Connected"
            self.current_height = int(daemon_info['height'])
            target_height = int(daemon_info['target_height'])
	...
	self.hub.update_daemon_status(json.dumps(info))
        ...
        sync_status = "Disconnected" if daemon_info['status'] != "OK" else "Synchronizing..."
        if daemon_info['status'] == "OK" and self.current_height == self.target_height:
            sync_status = "Connected"
        
        self.trayIcon.setToolTip("%s\n%s (%s/%s)" % (tray_icon_tooltip, sync_status,
    ...
    def about(self):
        QMessageBox.about(self, "About", \
            u"%s <br><br>Copyright© 2018 - The Fonero Projects (www.fonero.org)" % self.agent)
